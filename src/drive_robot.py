...
class Driver():
    ...
    def __init__(self):
        ...
        # initialise subscriber and subscribe to /sensed_object
        self.sub = rospy.Subscriber(
            '/sensed_object', Pose, self.parse_telemetry)
    ...
    def compute_drive_commands(self):
        ...
        alpha = math.degrees(-1*self.telemetry.orientation.z)
        xL = self.X_L
        yL = self.Y_L
        theta = self.Theta_L

        # implementing the control function
        self.matrix = np.array([[math.cos(math.radians(theta+alpha))+((yL/xL)*math.sin(math.radians(theta+alpha))), (-(math.sin(math.radians(theta+alpha)))+(
            yL/xL)*math.cos(math.radians(theta+alpha)))], [(1/xL)*math.sin(math.radians(theta+alpha)), (1/xL)*math.cos(math.radians(theta+alpha))]])

        self.AVL_matrix = np.array([[AVL_x], [AVL_y]])

        self.resultant_matrix = np.matmul(
            self.matrix, self.AVL_matrix)  # getting v and omega matrix

        self.drive_command.linear.x = float(self.resultant_matrix[0])  # v
        self.drive_command.angular.z = 0.4 * \
            float(self.resultant_matrix[1])  # omega
