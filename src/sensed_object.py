...
class LidarBaby():
    ...
    PUB_FREQ = 60  # Hz
    CCMVENT_DIST = 1  # Metres
    MAX_ERROR = 0.3  # 0.3 metres
    KP = 0.4  # Proportional Coefficient

    # Information to translate lidar frame (/base_laser) to robot frame (/base_link)
    X_L = 0.337  # Metres
    Y_L = 0  # Metres
    Theta_L = 0  # Radians
    ...
    def compute_telemetry(self, msg):
        ...
        # create numpy array to be able to use the magic of numpy
        self.np_lidar_ranges = np.array(self.lidar_data)
        # print(self.np_lidar_data)

        self.np_lidar_ranges[self.np_lidar_ranges >
                             msg.range_max] = float('NaN')
        self.np_lidar_ranges[self.np_lidar_ranges <
                             msg.range_min] = float('NaN')
        self.min_distance = np.nanmin(self.np_lidar_ranges)
        self.bearing_index = np.reshape(np.argwhere(
            self.np_lidar_ranges == self.min_distance), -1)
        ...
        try:
            # just pass the first min value
            self.first_bearing_index = self.bearing_index[0]
            # convert the bearing to usable degree value
            self.bearing = (self.first_bearing_index *
                            msg.angle_increment+msg.angle_min)*180.0/np.pi
            ...
            self.telemetry_data.orientation.z = math.radians(
                self.bearing)  # bearing to minimum distance
    ...
    def telemetry_publisher(self):
        self.telemetry_publisher_obj = rospy.Publisher(
            '/sensed_object', Pose, queue_size=10)
